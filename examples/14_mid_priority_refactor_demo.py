...
async def demo_unified_config():
    ...
    print(f"  ✓ Created config: agent.model={config1.agent.model.model if hasattr(config1.agent.model, 'model') else config1.agent.model}")
    print()
    ...
    config3 = OpenClawConfig.from_dict({
        "agent": {"model": "anthropic/claude-haiku"},
        "gateway": {"port": 9000}
    })
...
async def demo_method_registry():
    ...
    if connect_method:
        result = await connect_method.execute(connection, {
            "maxProtocol": 1,
            "client": {
                "name": "test-client",
                "version": "1.0.0",
